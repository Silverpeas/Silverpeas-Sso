 ...
 * {@link KerberosSpnegoFilter} javadoc.
 * </p>
 * <p/>
 * <p>
...
public final class SpnegoAuthenticator {
  ...
  public SpnegoAuthenticator(final SpnegoFilterConfig config)
    ...
    if (config.useKeyTab()) {
      ...
      final CallbackHandler handler = SpnegoProvider
          .getUsernamePasswordHandler(config.getPreauthUsername(), config.getPreauthPassword());
  ...
  public SpnegoPrincipal authenticate(final HttpServletRequest req,
    ...
    final SpnegoAuthScheme scheme =
        SpnegoProvider.negotiate(req, resp, basicSupported, this.promptIfNtlm, serverRealm);
    ...
    if (scheme.isNegotiateScheme()) {
      ...
      if (basicSupported) {
        ...
        logger().error("allowBasic=" + this.allowBasic + "; allowUnsecure=" + this.allowUnsecure +
  ...
  public void dispose() {
    if (null != this.serverCredentials) {
      try {
        ...
        logger().warn("Dispose failed. {0}", e.getMessage());
      }
    }
    if (null != this.loginContext) {
      try {
        ...
        logger().warn("Logout failed. {0}", le.getMessage());
      }
    }
  }
  ...
  private SpnegoPrincipal doBasicAuth(final SpnegoAuthScheme scheme,
    ...
    if (0 == data.length) {
      logger().debug("Basic Auth data was NULL.");
      return null;
    }
    ...
    if (basicData.length != 2) {
      throw new IllegalArgumentException(
          "Username/Password may" + " have contained an invalid character. basicData.length=" +
              basicData.length);
    }
    ...
    try {
      ...
      logger().info(
          le.getMessage() + ": Login failed. username=" + username + "; password.hashCode()=" +
              password.hashCode());
  ...
  private SpnegoPrincipal doLocalhost() {
    ...
    if (null == username || username.isEmpty()) {
      return new SpnegoPrincipal(
          this.serverPrincipal.getName() + '@' + this.serverPrincipal.getRealm(),
  ...
  private SpnegoPrincipal doSpnegoAuth(final SpnegoAuthScheme scheme,
    ...
    if (0 == gss.length) {
      logger().debug("GSS data was NULL.");
      return null;
    }
    ...
    try {
      ...
      if (!context.isEstablished()) {
        logger().debug("context not established");
        resp.setStatus(HttpServletResponse.SC_UNAUTHORIZED, true);
        return null;
      }
